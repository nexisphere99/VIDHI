# DAY 3 — IMAGE PROMPTS
## "Pehla Badlav" (First Change) — FIRST SWAP DAY
## Style: Semi-realistic, consistent characters, emotional intensity

---

## SCENE ILLUSTRATIONS

### IMG-200: Pataleshwar Temple — Pre-Dawn Swap
**File:** `pataleshwar_dawn.png`
**Insertion:** Day 3 opening, both characters
**Prompt:**
```
Ancient rock-cut cave temple courtyard at pre-dawn (5 AM), two young people sitting cross-legged facing each other inside a circle of red kumkum powder on stone floor, connected by a red thread on their wrists. One is a young man in a blue checked shirt, the other a young woman in maroon salwar kameez. A single sandalwood incense stick glows between them. The sky above is deep indigo fading to grey at the horizon. The ancient carved stone pillars frame them. The atmosphere is charged, mystical, the moment before transformation. Semi-realistic digital illustration, pre-dawn blue-indigo palette with warm kumkum-red and incense-gold accents.
```

### IMG-201: Arjun's First Moment in Kavya's Body
**File:** `arjun_first_kavya_body.png`
**Insertion:** `day3_arjun_main_scenes.md Scene 2`
**Prompt:**
```
A young Indian woman's body (Kavya — dusky skin, wavy hair, beauty mark) in the temple courtyard at dawn, but the EXPRESSION is wrong — the face wears masculine shock, eyes wide, one hand raised and stared at in disbelief, the other hand pressed against the chest feeling breasts for the first time. Thick wavy hair falls across the face unchecked. The maroon kameez drapes over a body being perceived from inside for the first time. Across from her/him, a young man's body (Arjun's) sits with clinical feminine composure. The contrast between body and expression is the key. Semi-realistic digital illustration, dawn golden light, the visual dissonance of wrong-soul-in-right-body.
```

### IMG-202: Kavya's First Moment in Arjun's Body
**File:** `kavya_first_arjun_body.png`
**Insertion:** `day3_kavya_main_scenes.md Scene 1`
**Prompt:**
```
A young Indian man's body (Arjun — wheat skin, stubble, lean build) in the temple courtyard at dawn, but the expression is sharp, analytical, feminine intelligence behind masculine features. He/she is looking down at large hands, spreading the fingers wide in wonder, examining the span. The posture is slightly too straight, too controlled — a dancer's posture in a swimmer's body. The blue checked shirt is slightly wrinkled. Behind him/her, a black Bajaj Pulsar motorcycle is parked. Semi-realistic digital illustration, dawn light, the visual of a mind discovering a new body with scientific fascination rather than masculine comfort.
```

### IMG-203: Kavya Riding the Pulsar
**File:** `kavya_pulsar_ride.png`
**Insertion:** `day3_kavya_main_scenes.md Scene 2`
**Prompt:**
```
A young Indian man (Arjun's body) riding a black Bajaj Pulsar 150 through empty Pune streets at dawn, leaning slightly forward, wind in short hair, expression of exhilaration and slight terror. The body language is subtly wrong for a male rider — the grip on handlebars slightly too precise, the posture too upright. Dawn light paints the road in orange-gold. Street lights still on. The road curves ahead. The speed is tangible — blur at the edges, the sense of velocity and freedom. Semi-realistic digital illustration, dynamic motion composition, dawn highway tones.
```

### IMG-204: Meera Reunion
**File:** `meera_reunion.png`
**Insertion:** `day3_arjun_main_scenes.md Scene 4`
**Prompt:**
```
Two young Indian women embracing in a hostel room — one (Meera — fair, loose hair, white kurta) crying into the other's hair, holding tight. The other (Kavya's body — dusky, wavy hair, maroon kameez) is also crying, face pressed against Meera's shoulder, but the EYES are different — carrying a masculine longing, six months of separation visible in the intensity. A third bed in the background is empty (Priya gone). Morning light through curtains. The embrace is desperate, tender, the geometry of two women's bodies holding the love of a man and a woman. Semi-realistic digital illustration, warm morning light, raw emotion, the beauty of reunion in borrowed form.
```

### IMG-205: Arjun's Anatomy Horror
**File:** `arjun_anatomy_horror.png`
**Insertion:** `day3_arjun_main_scenes.md Scene 6`
**Prompt:**
```
A young Indian woman (Kavya's body) in a white coat at a dissection table, dupatta pressed over nose, eyes streaming tears from formalin, expression of barely-contained nausea. She's holding a scalpel with steady hands despite the visible distress. Other students around appear unfazed. A kind elderly professor (Dr. Sharma) approaches from behind. The dissection table has a cloth-covered cadaver with only an arm region exposed. Semi-realistic digital illustration, harsh fluorescent anatomy-lab lighting, the clash between body's competence and mind's horror.
```

### IMG-206: Kavya in CS Lecture — The Brilliant Answer
**File:** `kavya_cs_lecture.png`
**Insertion:** `day3_kavya_main_scenes.md Scene 4`
**Prompt:**
```
A young Indian man (Arjun's body) standing in a college lecture hall, one hand raised, speaking with passionate clarity to a stunned room. The whiteboard behind shows "Red-Black Tree — Deletion Cases." A professor (Krishnan — mid-50s, sharp eyes) watches with raised eyebrows and a growing smile. Students around are frozen — one (Rohit, stocky, bearded) has turned fully around with mouth open. Another (Nikhil, glasses) leans forward in recognition. The speaker's expression carries feminine intellectual fire behind masculine features — the joy of finally being in the RIGHT room. Semi-realistic digital illustration, bright lecture hall lighting, the electric moment of unexpected brilliance.
```

### IMG-207: Kavya Coding Ecstasy
**File:** `kavya_coding_joy.png`
**Insertion:** `day3_kavya_main_scenes.md Scene 6`
**Prompt:**
```
A young Indian man (Arjun's body) sitting at a desk with a laptop and external monitor, hands on a mechanical keyboard, VS Code filling both screens with Python code. His expression is rapturous — eyes bright, slight smile, a single tear track on a stubbled cheek. The room around him is messy (PG room — Maggi cups, gym bag) but he's oblivious to everything except the code. Late afternoon golden light through the window. The visual of a person who has finally, after years of constraint, reached the place where they belong. Semi-realistic digital illustration, warm golden afternoon tones, the beauty of creative flow state.
```

### IMG-208: Rohit Canteen Confrontation — Day 3
**File:** `kavya_rohit_canteen.png`
**Insertion:** `day3_kavya_npc_sidequests.md → Rohit canteen scene`
**Prompt:**
```
Two young Indian men at a college canteen table. One (Rohit — stocky, bearded) leans forward with narrowed eyes, dosa torn in hand, expression between suspicious and impressed. The other (Arjun's body, Kavya inside) sits across, dosa plate between them, expression carefully casual — too carefully, the effort of performance visible in the set of the jaw. Chai glasses between them. Canteen background blurred. Semi-realistic digital illustration, warm canteen light, the tension of a best friend who senses something has changed.
```

---

## NEW LOCATION BACKGROUNDS

### IMG-210: Room 304 — Arjun's POV
**File:** `room_304_arjun_pov.png`
**Insertion:** `day3_arjun_hub.tw → hostel_room_304_d3`
**Prompt:**
```
Interior of hostel Room 304 but viewed with FRESH EYES — the familiar room rendered as discovery. Three beds, three cupboards, study desk with medical textbooks, but the focus is on Meera's bed (a woman sitting on it, waiting, white kurta) and the door just opening, morning light from the corridor falling across the stone floor. The atmosphere is anticipation — the room holds its breath. Semi-realistic digital illustration, warm morning tones filtering through corridor light, a familiar space made extraordinary by context.
```

### IMG-211: PG Room — Kavya's POV
**File:** `pg_room_kavya_pov.png`
**Insertion:** `day3_kavya_hub.tw → katraj_pg_room_d3`
**Prompt:**
```
Interior of the boys' PG room viewed with WONDER — the messy room (Maggi tower, gym bag, basketball jersey) but the focus is on the LAPTOP on the desk, screen glowing with VS Code, mechanical keyboard beside it, dual monitor setup. The laptop screen is almost holy — lit from within, the centerpiece of the image. A sleeping figure (Rohit) on a cot in background. The mess is irrelevant; the laptop is paradise. Semi-realistic digital illustration, the laptop screen as the brightest element, warm room tones, a coder's promised land.
```

---

## CHARACTER EXPRESSION VARIANTS — DAY 3

### IMG-220: Arjun-in-Kavya — Panic
**File:** `arjun_kavya_panic.png`
**Prompt:**
```
Portrait bust of Kavya's face (dusky, beauty mark, wavy hair) but wearing ARJUN'S panic — wide eyes, slightly open mouth, the expression of a masculine mind confronting feminine physicality. Hair messy, falling in face. The beauty mark is prominent. Semi-realistic, consistent with Kavya reference but WRONG expression.
```

### IMG-221: Arjun-in-Kavya — Tender
**File:** `arjun_kavya_tender.png`
**Prompt:**
```
Portrait bust of Kavya's face looking at someone (Meera) with Arjun's love — soft eyes, the specific tenderness of a man seeing the woman he loves after six months, but through another woman's features. Tears on cheeks. The dissonance of masculine devotion in feminine features. Semi-realistic, warm close-up tones.
```

### IMG-222: Kavya-in-Arjun — Wonder
**File:** `kavya_arjun_wonder.png`
**Prompt:**
```
Portrait bust of Arjun's face (wheat skin, stubble, long wavy hair) wearing Kavya's intellectual wonder — sharp focus, slight upward curve of lips, eyes bright with discovery. The expression is too controlled, too precise for Arjun's usually relaxed features. It's a woman's curiosity in a man's face. Semi-realistic.
```

### IMG-223: Kavya-in-Arjun — Coding Joy
**File:** `kavya_arjun_coding_joy.png`
**Prompt:**
```
Portrait bust of Arjun's face illuminated by monitor light, expression of pure unguarded joy — a rare single tear on stubbled cheek, eyes reflecting code on screen, the face of someone who has reached home after years of exile. Semi-realistic, blue-white screen glow on warm skin.
```

### IMG-224: Kavya-in-Arjun — Erection Panic
**File:** `kavya_arjun_erection_panic.png`
**Prompt:**
```
Portrait bust of Arjun's face with an expression of startled alarm — eyes widened, slight blush on wheat cheeks, the specific look of someone experiencing an unexpected physical event they can't control. The panic is distinctly feminine — more bewildered than embarrassed. Semi-realistic, warm room lighting. Tasteful — the expression tells the story.
```

---

## ADDED IN IMPLEMENTATION — extra Day 3 scenes

These passages were written after this prompt doc and the game already
references the images below. Art added and wired.

### IMG-212: Arjun's PG Room — Pre-Dawn
**File:** `katraj_pg_night.png`
**Insertion:** `arjun_predawn_d3` (Day 3, ~4:00 AM, Arjun's own body)
**Prompt:**
```
A cramped Indian PG room at 4 AM, near-total dark cut only by a phone screen and a sodium streetlight through a thin curtain. One cot holds a mound of blanket and a sleeping boy (Rohit); the other is empty, sheet thrown back. A riding jacket hangs on a plastic chair, a motorcycle key held in a hand at the edge of frame. Axe deodorant, stale Maggi cups, a gym bag. Cold blue-black palette, one warm phone-glow accent. Semi-realistic digital illustration, the stillness of a room someone is about to leave before dawn.
```

### IMG-213: Room 304 — Pre-Dawn, Meera Awake
**File:** `hostel_304_night.png`
**Insertion:** `kavya_predawn_d3` (Day 3, ~3:55 AM, Kavya's own body)
**Prompt:**
```
A three-bed Indian girls' hostel room at 3:55 AM. Two roommates asleep (one flat on her back, notes on her chest; one a comma under blankets). On the third bed a young woman (Meera, white kurta) sits up against the wall, knees to her chest, awake, watching. A folded maroon salwar kameez and a dupatta are laid over a chair. Faint grey pre-dawn at the window, one small nightlight. Cold palette, quiet, the ache of someone who has to stay behind. Semi-realistic digital illustration.
```

### IMG-214: The Ghat Road to Pataleshwar — Pre-Dawn
**File:** `pataleshwar_approach.png`
**Insertion:** `arjun_ride_temple_d3` AND `kavya_walk_temple_d3`
**Prompt:**
```
An empty two-lane ghat road winding uphill toward a dark cave-temple hill before dawn. Sodium streetlights, a single motorcycle headlight cutting the frame (or, alternate crop, a lone figure in a salwar walking with a dupatta pulled over the head). Shuttered shopfronts, a stray dog, one milk van. The sky is just beginning to thin at the horizon, deep indigo overhead. Semi-realistic digital illustration, cold pre-dawn palette with a warm headlight/streetlight accent, the loneliness of the hour before a city wakes.
```

### IMG-215: The Handover — Just After the Swap
**File:** `day3_briefing.png`
**Insertion:** `day3_briefing` (immediately after `day3_swap`)
**Prompt:**
```
Two young people sitting knee to knee inside a circle of red kumkum on ancient temple stone, grey dawn coming up behind the caves. The visual trick: each wears the OTHER's face on the wrong body — the young man's body (Arjun's) with a woman's sharp composure and a folded sheet of ruled paper in hand; the young woman's body (Kavya's) with a man's stunned, coming-apart expression, one small ringed hand pressed to its own face. They are mid-conversation, one talking, one nodding. A single oil lamp. Semi-realistic digital illustration, pre-dawn blue with kumkum-red and lamp-gold, the fifteen-minute crash course.
```

### IMG-216: Time Alone with Meera
**File:** `meera_close.png`
**Insertion:** `meera_close_d3` (Day 3 afternoon, Room 304, Priya out)
**Prompt:**
```
Room 304, door bolted, warm afternoon light. Two women on a narrow single cot — one (Meera, white kurta, undone braid) holding the other, who is curled small against her chest. The held one has Kavya's body and face but the posture and the eyes are a man's — the geometry inverted, he is the small one now. Tender, not sexual: a hand in hair, foreheads close, the exhaustion of six months apart finally set down. A folded palm-leaf manuscript on the desk. Semi-realistic digital illustration, warm amber afternoon tones, the emotional core of the whole day.
```

### IMG-217: VIT Library — The Algorithms Aisle
**File:** `vit_library_cs.png`
**Insertion:** `kavya_library_d3` (Day 3, Kavya in Arjun's body)
**Prompt:**
```
The computer-science aisle of a university library — cool, carpeted, hushed. Floor-to-ceiling shelves of hardcover algorithms and systems texts (CLRS, Sedgewick, the Dragon Book, bound conference proceedings). A young man (Arjun's body) stands holding one heavy hardcover open, expression of quiet want fulfilled — a woman's reverence in a man's face. Fluorescent light, a window with late-morning sun at the aisle's end. Semi-realistic digital illustration, cool library palette with one warm window accent, the feeling of finally touching the real thing after years of bad PDFs.
```

### IMG-218: The CS Lab Doorway
**File:** `vit_cs_lab_doorway.png`
**Insertion:** `kavya_cs_lab_first_d3` (Day 3, Kavya in Arjun's body)
**Prompt:**
```
POV / over-shoulder from the doorway of a proper university CS computer lab: rows of dual-monitor Dell machines, good mechanical keyboards, ceiling projectors, a whiteboard wall, one air-conditioner. A lab assistant at a desk not looking up. In the foreground edge, a young man (Arjun's body) pausing on the threshold — you read it in the shoulders — before stepping in. The room is ordinary; the moment is not. Semi-realistic digital illustration, cool even lab lighting, the hush before someone enters the room they were always aimed at.
```

---

## TOTAL IMAGE COUNT — DAY 3

```
Scene Illustrations:              9  (IMG-200 to IMG-208)
New Location Backgrounds:         2  (IMG-210 to IMG-211)
Expression Variants:              5  (IMG-220 to IMG-224)   [not wired   future gallery]
Implementation additions:        7  (IMG-212 to IMG-218)   [wired, art pending]
─────────────────────────────────────
DAY 3 NEW IMAGES:                23

CUMULATIVE (Day 1 + 2 + 3):    108 images

PRIORITY ORDER:
1. Swap scene (IMG-200) — the defining moment
2. First-body-check images (IMG-201, 202) — the visual WOW
3. Reunion (IMG-204) — emotional core
4. CS lecture brilliance (IMG-206) — Kavya's triumph
5. Coding ecstasy (IMG-207) — the emotional payoff
6. Pulsar ride (IMG-203) — freedom visual
7. Anatomy horror (IMG-205) — comedy/contrast
8. Expression variants — dialogue enhancement
```

---

## STYLE NOTES — DAY 3

- **THE KEY VISUAL CHALLENGE**: showing the WRONG expression on the RIGHT face. Kavya's face with Arjun's emotions. Arjun's face with Kavya's intelligence. The body is correct; the soul behind the eyes is not. This dissonance IS the visual story.
- **Expression artists**: study how the same face changes when different personalities inhabit it. The jaw set, the eye focus, the tension in the brow — these micro-expressions tell the viewer that the person inside doesn't match the person outside.
- **The SWAP scene** should feel mythic — ancient stone, red kumkum, pre-dawn blue. This is a sacred act in a sacred space.
- **The CODING scene** should feel intimate — a person crying at a keyboard is not sad, it's beautiful. The tears are joy. The light is warm. The code on screen is the most beautiful thing in the image.
- **The REUNION** should be devastating — two women's bodies holding a man's love. The viewer should feel the six months of separation in the grip of the embrace.
- **Body scenes** (first check, bathroom) are NOT depicted visually for safety — they exist in text only. The images capture the EMOTIONAL moments, not the physical discovery.
